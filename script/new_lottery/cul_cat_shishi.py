...
def get_haoma_from_sql(issue_no):
    ...
    db = pymysql.connect('fusion.mysql.polardb.rds.aliyuncs.com', 'dscp', pwd,
                         'dscp', charset='utf8')
    cursor = db.cursor()
    cursor.execute('SELECT lottery_issue_prize_num FROM ds_lottery_issue WHERE lottery_id = (SELECT lottery_id FROM ds_lottery WHERE lottery_name = "五分时时牛牛") AND lottery_issue_no = %s;' % issue_no)
...
def gen_num(li):
    ...
    if len(li) != 5:
        ...
        li = li + li
        for i in range(5):
            group.append(li[i:i+4])
    return group


while(1):
    try:
        issue_no = int(input('请输入五分时时牛牛的期号：'))
